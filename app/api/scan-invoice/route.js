  ...
  "line_items": [
    {
      ...
      "item_number": "item number or SKU as printed on the invoice, null if not visible",
      "qty": 0,
...
- item_number should be the distributor SKU or item code printed on the invoice line — look for numeric or alphanumeric codes near the product name
...
- Return ONLY the JSON, nothing else`
    ...
    const { data: inventoryItems } = await supabase
      ...
      .select('id, name, unit, unit_cost, item_number')
      .eq('user_id', userId)
      .eq('area', area)
    ...
    const matchedItems = (invoiceData.line_items || []).map(item => {
      ...
      if (inventoryItems) {
        // Step 1 — SKU/item number exact match first
        if (item.item_number) {
          const skuMatch = inventoryItems.find(inv =>
            inv.item_number &&
            inv.item_number.trim().toLowerCase() === item.item_number.trim().toLowerCase()
          )
          if (skuMatch) {
            bestMatch = skuMatch
            bestScore = 1.0
        ...
        // Step 2 — Fall back to fuzzy name match
        if (!bestMatch) {
      ...
      return {
        ...
        matched_item_number: bestMatch?.item_number || null,
        match_confidence: bestScore,
        match_method: matchMethod,
        match_status: bestMatch
          ? (matchMethod === 'sku' || bestScore > 0.7 ? 'matched' : 'low_confidence')
          : 'unmatched'
      }
    })
