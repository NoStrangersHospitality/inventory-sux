  ...
  useEffect(() => {
    const init = async () => {
      ...
      const [{ count: itemCount }, { count: distCount }, { data: orderData }, { data: pendingData }] = await Promise.all([
        ...
        supabase.from('orders').select('*, order_lines(count)').eq('user_id', session.user.id).not('area', 'eq', 'boh').eq('receiving_status', 'received').order('submitted_at', { ascending: false }).limit(10),
        supabase.from('orders').select('*, order_lines(*)').eq('user_id', session.user.id).not('area', 'eq', 'boh').eq('receiving_status', 'pending').order('submitted_at', { ascending: true })
      ])
