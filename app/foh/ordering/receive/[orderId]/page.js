...
export default function ReceiveOrder() {
  ...
  const processReceiving = async (status, resolvedLines) => {
    ...
    for (const line of resolvedLines) {
      const lineStatus = !line.received ? 'missing' : line.received_qty < line.final_qty ? 'short' : 'received'
      await supabase.from('order_lines').update({
        received_qty: parseFloat(line.received_qty) || 0,
        receiving_status: lineStatus
      }).eq('id', line.id)

      // Update inventory on_hand for received items
    ...
    // Clear session banner dismissal so it recalculates
    ...
    <div style={{ minHeight: '100vh', background: '#f5f5f3', fontFamily: '-apple-system, BlinkMacSystemFont, Segoe UI, sans-serif' }}>
      ...
      <div style={{ padding: '28px 24px', maxWidth: '900px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px' }}>
          <div>
            <h1 style={{ fontSize: '20px', fontWeight: '500', color: '#000' }}>Confirm Delivery</h1>
        ...
        <div style={{ background: '#f0f8ff', border: '1px solid #b5d4f4', borderRadius: '10px', padding: '12px 16px', marginBottom: '20px', fontSize: '12px', color: '#185FA5' }}>
          💡 All items are checked by default. Uncheck anything that didn't arrive, and adjust quantities for short shipments. Confirmed items will update your inventory automatically.
        </div>
        ...
        {/* Line items grouped by distributor */}
        {Object.entries(grouped).map(([distName, distLines]) => (
          <div key={distName} style={{ marginBottom: '20px' }}>
