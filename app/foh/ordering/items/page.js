...
export default function Items() {
  ...
  const loadData = async (userId) => {
    const [{ data: invItems }, { data: dists }] = await Promise.all([
      supabase.from('inventory_items').select('*').eq('user_id', userId).eq('area', 'foh').eq('on_menu', true).order('name'),
  ...
  const openForm = (item = null) => {
    if (item) {
      setForm({ name: item.name, category: item.category, item_type: item.item_type || 'bottle', unit: item.unit || 'bottle', unit_cost: item.unit_cost || '', par: item.par || '', distributor_id: item.distributor_id || '', notes: item.notes || '', on_hand: item.on_hand || '' })
      setEditingId(item.id)
    } else {
      setForm({ name: '', category: activeCategory, item_type: ITEM_TYPES[activeCategory][0], unit: 'bottle', unit_cost: '', par: '', distributor_id: '', notes: '', on_hand: '' })
      setEditingId(null)
    }
    setShowForm(true)
  }
  ...
  const saveItem = async () => {
    ...
    const payload = {
      ...
      par: parseFloat(form.par) || 0, distributor_id: form.distributor_id || null,
      notes: form.notes, on_hand: parseFloat(form.on_hand) || 0,
      on_menu: true, area: 'foh', user_id: session.user.id
    }
    if (editingId) {
  ...
  const downloadTemplate = () => {
    const examples = {
      liquor: [['Buffalo Trace Bourbon', 'bottle', 'bottle', '35.10', '6', '0', 'Republic National', '750ml / 12 case']],
      beer: [["Hamm's 24pk", 'can', 'case', '18.00', '4', '0', 'Republic National', '']],
    ...
    reader.onload = (ev) => {
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        parsed.push({
          ...
          distributor_id: dist?.id || null, distName,
  ...
  const confirmImport = async () => {
    ...
    if (toInsert.length > 0) {
      await supabase.from('inventory_items').insert(toInsert.map(r => ({
        ...
        notes: r.notes, on_menu: true, area: 'foh', user_id: session.user.id
      })))
      await loadData(session.user.id)
    }
    ...
    <div style={{ minHeight: '100vh', background: '#f5f5f3', fontFamily: '-apple-system, BlinkMacSystemFont, Segoe UI, sans-serif' }}>
      ...
      <div style={{ padding: isMobile ? '16px' : '28px 24px', maxWidth: '1000px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '14px', gap: '12px' }}>
          <div>
            <h1 style={{ fontSize: isMobile ? '17px' : '20px', fontWeight: '500', color: '#000' }}>Items</h1>
            {!isMobile && <p style={{ color: '#999', fontSize: '13px', marginTop: '4px' }}>Items marked On Menu in your inventory database</p>}
          </div>
        ...
        <div style={{ background: '#f0f8ff', border: '1px solid #b5d4f4', borderRadius: '8px', padding: '10px 14px', marginBottom: '14px', fontSize: '12px', color: '#185FA5' }}>
          💡 Items appear here when marked <strong>On Menu</strong> in the Inventory Database.
        </div>
          ...
          {CATEGORIES.map(c => {
            ...
            return (
              <div key={c.key} onClick={() => { setActiveCategory(c.key); setImportPreview(null) }}
                ...
                <div style={{ fontSize: isMobile ? '20px' : '26px', marginBottom: '4px' }}>{c.icon}</div>
          ...
          <div style={{ background: '#fff', border: '1px solid #e8e8e8', borderRadius: '12px', padding: '16px', marginBottom: '16px' }}>
            ...
            {isMobile ? (
                  ...
                  return (
                    <div key={i} style={{ background: '#fafafa', borderRadius: '8px', padding: '10px 12px', border: '1px solid #f0f0f0' }}>
                      ...
                      <div style={{ fontSize: '11px', color: '#aaa' }}>
                        {r.item_type} · ${r.unit_cost} · par {r.par}
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px' }}>
                <thead>
                  <tr>{['Product', 'Type', 'Unit Cost', 'Par', 'Distributor', 'Status'].map((h, i) => (
                  ...
                  <div key={item.id} style={{ background: '#fff', border: '1px solid #e8e8e8', borderRadius: '12px', padding: '14px 16px' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '8px' }}>
                      <div>
                        ...
                        <div style={{ fontSize: '13px', fontWeight: '500', color: '#555' }}>${Number(item.unit_cost).toFixed(2)}</div>
                      </div>
                      <div>
                        <div style={{ fontSize: '10px', color: '#aaa', marginBottom: '2px', textTransform: 'uppercase' }}>On Hand</div>
                        <div style={{ fontSize: '13px', fontWeight: '700', color: isLow ? '#E24B4A' : '#000' }}>{Number(item.on_hand).toFixed(1)}</div>
                      </div>
                      <div>
