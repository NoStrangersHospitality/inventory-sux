          ...
          <>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px', gap: '12px' }}>
              <div>
                ...
                {!isMobile && <p style={{ color: '#999', fontSize: '13px', marginTop: '4px' }}>Scan a delivery invoice, review the lines, then approve to keep it on record. Receiving confirmation is what updates on hand quantities.</p>}
              </div>
            ...
            {invoices.length === 0 ? (
              <div style={{ background: '#fff', border: '1px solid #e8e8e8', borderRadius: '12px', padding: '48px', textAlign: 'center', color: '#ccc', fontSize: '14px' }}>
                No invoices scanned yet. Upload your first delivery invoice above.
              </div>
            ) : isMobile ? (
        ...
        {view === 'review' && scanResult && (
          <>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '16px', gap: '12px' }}>
              <div>
                <h1 style={{ fontSize: isMobile ? '17px' : '20px', fontWeight: '500', color: '#000' }}>Review Invoice</h1>
            ...
            <div style={{ background: '#fffbe6', border: '1px solid #f0d060', borderRadius: '8px', padding: '10px 14px', marginBottom: '14px', fontSize: '12px', color: '#a07800' }}>
              💡 Review and match items below, then hit <strong>Save for Approval</strong>. This is a record for your books — receiving confirmation is what updates on hand quantities.
            </div>
            ...
            {isMobile ? (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                {(scanResult.line_items || []).map((line, idx) => (
                  <div key={idx} style={{ background: line.match_status === 'matched' ? '#f9fff5' : '#fff', border: `1px solid ${line.match_status === 'matched' ? '#97C459' : '#e8e8e8'}`, borderRadius: '12px', padding: '14px 16px' }}>
                    ...
                    <div>
                      <div style={{ fontSize: '10px', color: '#aaa', marginBottom: '4px', textTransform: 'uppercase' }}>Match to Inventory</div>
              ...
              <div style={{ background: '#fff', border: '1px solid #e8e8e8', borderRadius: '12px', overflow: 'hidden' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  <thead>
                    <tr>{['Invoice Line Item', 'Qty', 'Unit Cost', 'Total', 'Match to Inventory Item', 'Status'].map((h, i) => (
                      <th key={i} style={{ textAlign: 'left', fontSize: '11px', color: '#aaa', textTransform: 'uppercase', letterSpacing: '.4px', padding: '10px 14px', borderBottom: '1px solid #f0f0f0', background: '#fafafa' }}>{h}</th>
                    ))}</tr>
                  </thead>
                  <tbody>
            ...
            <div style={{ marginTop: '12px', fontSize: '12px', color: '#aaa' }}>
              Saving keeps this as a record but does not touch inventory. Receiving confirmation is what updates on hand quantities — approving here just marks the invoice reviewed.
            </div>
          </>
        )}
      </div>
