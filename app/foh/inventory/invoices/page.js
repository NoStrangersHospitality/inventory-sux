...
total_value_at_time: (parseFloat(line.qty) || 0) * (line.case_size || 1) * (line.case_size > 1 ? (parseFloat(line.unit_cost) || 0) / (line.case_size || 1) : (parseFloat(line.unit_cost) || 0))
        ...
        await supabase.from('inventory_history').insert({
          ...
          quantity_change: parseFloat(line.qty) || 0,
        ...
        {view === 'hub' && (
          <>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '24px' }}>
              <div>
                <h1 style={{ fontSize: '20px', fontWeight: '500', color: '#000' }}>Invoice Scanning</h1>
                <p style={{ color: '#999', fontSize: '13px', marginTop: '4px' }}>Scan a delivery invoice to automatically update on hand quantities.</p>
              </div>
        ...
        {view === 'review' && scanResult && (
          <>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px' }}>
              <div>
                <h1 style={{ fontSize: '20px', fontWeight: '500', color: '#000' }}>Review Invoice</h1>
            ...
            <div style={{ background: '#f0f8ff', border: '1px solid #b5d4f4', borderRadius: '8px', padding: '10px 14px', marginBottom: '16px', fontSize: '12px', color: '#185FA5' }}>
              💡 Use the dropdown to match each line item to your inventory. Select <strong>+ Create new item</strong> to add items that don't exist yet. Unmatched items won't update inventory.
            </div>
  ...
  <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
    ...
    {line.case_size > 1 && (
      <div style={{ fontSize: '10px', color: '#185FA5', background: '#ddeeff', borderRadius: '4px', padding: '3px 6px' }}>
        {line.qty} case × {line.case_size} = <strong>{(parseFloat(line.qty) || 0) * line.case_size} units</strong> in inventory
      </div>
    )}
  </div>
)}
