...
const EditForm = ({ form, setForm, saving, isMobile, distributors, onSave, onCancel }) => {
    ...
    <div style={{ background: '#fafafa', borderLeft: isMobile ? 'none' : '3px solid #F5B800', borderTop: isMobile ? '3px solid #F5B800' : 'none', padding: '16px' }}>
      <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: '12px', marginBottom: '12px' }}>
        ...
        <div style={{ gridColumn: '1/-1' }}>
          ...
          <input style={inputStyle} placeholder="Distributor item number..." value={form.item_number} onChange={e => setForm(f => ({ ...f, item_number: e.target.value }))} />
        </div>
        <div>
        ...
        {form.category === 'wine' && (
          <div>
            ...
            <select style={inputStyle} value={form.wine_type || ''} onChange={e => setForm(f => ({ ...f, wine_type: e.target.value }))}>
              ...
              {['red', 'white', 'bubbles', 'rose', 'orange', 'other'].map(t => <option key={t} value={t}>{t.charAt(0).toUpperCase() + t.slice(1)}</option>)}
            </select>
          </div>
        )}
        <div>
        ...
        <div style={{ gridColumn: '1/-1', display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 12px', background: '#fff', borderRadius: '8px', border: '1px solid #e8e8e8' }}>
          ...
          <span style={{ fontSize: '13px', color: '#000' }}>On Menu — include on order sheet</span>
        </div>
      </div>
...
export default function FOHInventoryDatabase() {
  ...
  const openAddForm = () => {
    setForm({ name: '', category: activeCategory, item_type: ITEM_TYPES[activeCategory][0], on_hand: '', unit: '', unit_cost: '', par: '', on_menu: false, distributor_id: '', notes: '', wine_type: '', item_number: '' })
    setEditingId(null)
    setShowAddForm(true)
  }

  const openEditForm = (item) => {
    setForm({ name: item.name, category: item.category, item_type: item.item_type || 'bottle', on_hand: item.on_hand, unit: item.unit || '', unit_cost: item.unit_cost, par: item.par, on_menu: item.on_menu || false, distributor_id: item.distributor_id || '', notes: item.notes || '', wine_type: item.wine_type || '', item_number: item.item_number || '' })
  ...
  const saveItem = async () => {
    ...
    const payload = {
      ...
      item_number: form.item_number || null, area: 'foh', user_id: session.user.id
    }
    if (editingId) {
      ...
      if (original && parseFloat(form.on_hand) !== original.on_hand) {
        await supabase.from('inventory_history').insert({
          user_id: session.user.id, inventory_item_id: editingId, item_name: form.name,
          category: form.category, area: 'foh', event_type: 'manual_adjustment',
          quantity_before: original.on_hand, quantity_change: parseFloat(form.on_hand) - original.on_hand,
          quantity_after: parseFloat(form.on_hand), unit_cost_at_time: parseFloat(form.unit_cost) || 0,
  ...
  const downloadTemplate = () => {
    const examples = {
      liquor: [['083126', 'Buffalo Trace Bourbon', 'liquor', 'bottle', '12', 'bottle', '35.10', '6', 'Yes', 'Republic National', '']],
      beer: [['', "Hamm's 24pk", 'beer', 'can', '4', 'case', '18.00', '2', 'Yes', 'Republic National', '']],
  ...
  const handleImport = (e) => {
    ...
    reader.onload = (ev) => {
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        parsed.push({
          item_number: cols[ini >= 0 ? ini : 0] || '', name, category: activeCategory,
          ...
          distributor_id: dist?.id || null, distName, distMatched: !distName || !!dist,
  ...
  const confirmImport = async () => {
    ...
    if (toInsert.length > 0) {
      const { data: insertedItems, error: insertError } = await supabase.from('inventory_items').insert(toInsert.map(r => ({
        item_number: r.item_number || null, name: r.name.trim(), category: r.category,
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f5f5f3', fontFamily: '-apple-system, BlinkMacSystemFont, Segoe UI, sans-serif' }}>
      ...
      <div style={{ background: '#fff', borderBottom: '2px solid #F5B800', padding: isMobile ? '10px 16px' : '10px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <button onClick={() => router.push('/foh/inventory')} style={{ background: '#333', border: 'none', color: '#fff', padding: '6px 12px', borderRadius: '6px', fontSize: '12px', cursor: 'pointer' }}>← Inventory</button>
      </div>
      ...
      <div style={{ padding: isMobile ? '16px' : '28px 24px', maxWidth: '1100px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '16px', gap: '12px' }}>
          <div>
            <h1 style={{ fontSize: isMobile ? '17px' : '20px', fontWeight: '500', color: '#000' }}>Inventory Database</h1>
            {!isMobile && <p style={{ color: '#999', fontSize: '13px', marginTop: '4px' }}>Every item you own — on hand, value, par, and ordering status.</p>}
          </div>
          ...
          {CATEGORIES.map(c => {
            ...
            return (
              <div key={c.key} onClick={() => { setActiveCategory(c.key); setImportPreview(null); cancelEdit() }}
                style={{ background: '#fff', border: `2px solid ${activeCategory === c.key ? '#F5B800' : '#e8e8e8'}`, borderRadius: '12px', padding: isMobile ? '10px 6px' : '14px', cursor: 'pointer', textAlign: 'center' }}>
                <div style={{ fontSize: isMobile ? '20px' : '24px', marginBottom: '4px' }}>{c.icon}</div>
                <div style={{ fontSize: isMobile ? '11px' : '13px', fontWeight: '600', color: '#000', marginBottom: '2px' }}>{c.label}</div>
                {!isMobile && <div style={{ fontSize: '10px', color: '#aaa' }}>{count} · {fmt(val)}</div>}
                {isMobile && <div style={{ fontSize: '10px', color: '#aaa' }}>{count}</div>}
              </div>
            )
          })}
        </div>
        ...
        {showAddForm && (
          <div style={{ background: '#fff', border: '1px solid #e8e8e8', borderRadius: '12px', marginBottom: '16px', overflow: 'hidden' }}>
            ...
            <EditForm form={form} setForm={setForm} saving={saving} isMobile={isMobile} distributors={distributors} onSave={saveItem} onCancel={cancelEdit} />
          </div>
        )}
          ...
          <div style={{ background: '#fff', border: '1px solid #e8e8e8', borderRadius: '12px', padding: '16px', marginBottom: '16px' }}>
                  ...
                  return (
                    <div key={i} style={{ background: '#fafafa', borderRadius: '8px', padding: '10px 12px', border: '1px solid #f0f0f0' }}>
                      ...
                      <div style={{ fontSize: '11px', color: '#aaa' }}>
                        {r.item_type} · {r.on_hand} on hand · {r.unit_cost ? fmt(r.unit_cost) : '--'}
                        {r.distName && <span style={{ color: r.distMatched ? '#aaa' : '#E24B4A' }}> · {r.distName}{!r.distMatched ? ' ⚠' : ''}</span>}
                      </div>
                    </div>
                  )
                })}
            ...
            {importPreview.some(r => !r.distMatched && r.distName) && (
              <div style={{ marginTop: '10px', fontSize: '11px', color: '#854F0B', background: '#FAEEDA', border: '1px solid #f0c080', borderRadius: '6px', padding: '8px 12px' }}>
                ⚠ Unmatched distributors will be left blank.
              </div>
            )}
          </div>
        )}
                ...
                {catItems.map(item => {
                  ...
                  return (
                    <div key={item.id} style={{ background: '#fff', border: `1px solid ${isEditing ? '#F5B800' : '#e8e8e8'}`, borderRadius: '12px', overflow: 'hidden' }}>
                      <div style={{ padding: '14px 16px' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '8px' }}>
                          <div style={{ flex: 1, marginRight: '12px' }}>
                            ...
                            <div style={{ fontSize: '11px', color: '#aaa' }}>
                              {item.item_type}{dist && ` · ${dist.name}`}{item.item_number && ` · #${item.item_number}`}
                            </div>
                          </div>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '8px' }}>
                          <div>
                            ...
                            <div style={{ fontSize: '14px', fontWeight: '700', color: isLow ? '#E24B4A' : '#000' }}>{Number(item.on_hand).toFixed(1)}</div>
                          </div>
                          <div>
                            <div style={{ fontSize: '10px', color: '#aaa', marginBottom: '2px', textTransform: 'uppercase' }}>Cost</div>
                            <div style={{ fontSize: '13px', fontWeight: '500', color: '#555' }}>{fmt(item.unit_cost)}</div>
                          </div>
                          <div>
                            <div style={{ fontSize: '10px', color: '#aaa', marginBottom: '2px', textTransform: 'uppercase' }}>Value</div>
                            <div style={{ fontSize: '13px', fontWeight: '600', color: '#000' }}>{fmt(item.on_hand * item.unit_cost)}</div>
                          </div>
                          <div>
                            <div style={{ fontSize: '10px', color: '#aaa', marginBottom: '2px', textTransform: 'uppercase' }}>Par</div>
                            <div style={{ fontSize: '13px', fontWeight: '500', color: item.par > 0 && variance < 0 ? '#E24B4A' : '#555' }}>
                              {item.par > 0 ? `${Number(item.par).toFixed(1)} (${variance >= 0 ? '+' : ''}${variance.toFixed(1)})` : '--'}
                            </div>
                          </div>
                        </div>
                      ...
                      {isEditing && <EditForm form={form} setForm={setForm} saving={saving} isMobile={isMobile} distributors={distributors} onSave={saveItem} onCancel={cancelEdit} />}
                    </div>
                  )
                })}
              </div>
                        ...
                        <>
                          <tr key={item.id} style={{ borderBottom: isEditing ? 'none' : '1px solid #f5f5f5', background: isEditing ? '#fffdf0' : 'transparent' }}>
                            ...
                            <td style={{ padding: '10px 12px', textAlign: 'right', fontSize: '12px' }}>
                              {item.par > 0 ? <span style={{ color: variance >= 0 ? '#3B6D11' : '#E24B4A', fontWeight: '500' }}>{variance >= 0 ? '+' : ''}{variance.toFixed(1)}</span> : <span style={{ color: '#aaa' }}>--</span>}
                            </td>
