...
export default function FOHInventory() {
  ...
  const tiles = [
    { icon: '🗄️', title: 'Database', desc: 'Every item you own. On hand, value, par.', sub: `${stats.totalItems} items · ${fmt(stats.totalValue)} value`, href: '/foh/inventory/database' },
    { icon: '📋', title: 'Count', desc: 'Start a new count or review history.', sub: stats.lowStock > 0 ? `${stats.lowStock} items below par` : 'All items at or above par', subColor: stats.lowStock > 0 ? '#E24B4A' : null, href: '/foh/inventory/count' },
    { icon: '📄', title: 'Invoices', desc: 'Scan delivery invoices to update on hand.', sub: 'Photo or PDF · AI powered', href: '/foh/inventory/invoices' },
  ]
    ...
    <div style={{ minHeight: '100vh', background: '#f5f5f3', fontFamily: '-apple-system, BlinkMacSystemFont, Segoe UI, sans-serif' }}>
      ...
      <div style={{ padding: isMobile ? '20px 16px' : '28px 24px', maxWidth: '1000px', margin: '0 auto' }}>
        ...
        <div style={{ marginBottom: '20px' }}>
          <h1 style={{ fontSize: isMobile ? '18px' : '20px', fontWeight: '500', color: '#000' }}>FOH Inventory</h1>
