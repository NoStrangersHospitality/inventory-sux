...
export default function FOHReports() {
  ...
  const loadSessions = async (userId) => {
    ...
    // Auto-select the two most recent for default report
    if (data && data.length >= 2) {
      setSessionA(data[0].id)
      setSessionB(data[1].id)
    }

    // Build trend data from all sessions
  ...
  const runReport = async () => {
    ...
    // Ensure A is the later session, B is the earlier
    ...
    // Get invoices received between the two count dates
    ...
    // Sum invoice received amounts per item
    ...
    // Aggregate earlier lines by item
    const earlierTotals = {}
    ...
    // Build variance and usage report
    ...
    <div style={{ minHeight: '100vh', background: '#f5f5f3', fontFamily: '-apple-system, BlinkMacSystemFont, Segoe UI, sans-serif' }}>
      ...
      <div style={{ padding: '28px 24px', maxWidth: '1100px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '24px' }}>
          <div>
            <h1 style={{ fontSize: '20px', fontWeight: '500', color: '#000' }}>FOH Reports</h1>
                  ...
                  <div style={{ background: '#fff', border: '1px solid #e8e8e8', borderRadius: '12px', padding: '20px 24px', marginBottom: '20px' }}>
                    ...
                    <ResponsiveContainer width="100%" height={180}>
                      <BarChart data={categoryTrend} margin={{ left: 0, right: 16, top: 0, bottom: 0 }}>
                        <XAxis dataKey="category" tick={{ fontSize: 12, fill: '#555' }} axisLine={false} tickLine={false} />
                        <YAxis tickFormatter={v => '$' + v} tick={{ fontSize: 11, fill: '#aaa' }} axisLine={false} tickLine={false} width={56} />
