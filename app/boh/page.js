...
export default function BOH() {
  ...
  const modules = [
    ...
    { title: 'Reports', desc: 'Variance, usage, and cost trends', icon: '📊', href: '/boh/reports' },
  ]
              ...
              <div style={{ background: '#fff', border: '1px solid #e8e8e8', borderRadius: '12px', padding: '20px 24px' }}>
                ...
                <ResponsiveContainer width="100%" height={200}>
                  <BarChart data={categoryData} layout="vertical" margin={{ left: 0, right: 16, top: 0, bottom: 0 }}>
                    ...
                    <YAxis type="category" dataKey="name" tick={{ fontSize: 11, fill: '#555' }} axisLine={false} tickLine={false} width={64} />
        ...
        {topVariance.length > 0 && (
          <div style={{ background: '#fff', border: '1px solid #e8e8e8', borderRadius: '12px', padding: '20px 24px', marginBottom: '24px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px' }}>
              ...
              <button onClick={() => router.push('/boh/reports')}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px' }}>
          {modules.map(mod => (
            <div key={mod.title}
              ...
              style={{ background: '#fff', border: '1px solid #e8e8e8', borderRadius: '16px', padding: '28px 16px', cursor: 'pointer', textAlign: 'center', transition: 'border-color .15s, box-shadow .15s' }}
