...
export default function BOHOrdering() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      const [
        ...
        supabase.from('orders').select('*, order_lines(count)').eq('user_id', session.user.id).eq('area', 'boh').eq('status', 'submitted').order('submitted_at', { ascending: false }).limit(20),
        supabase.from('orders').select('*, order_lines(*)').eq('user_id', session.user.id).eq('area', 'boh').eq('status', 'submitted').eq('receiving_status', 'pending').order('submitted_at', { ascending: true }),
        supabase.from('orders').select('*, order_lines(count)').eq('user_id', session.user.id).eq('area', 'boh').eq('status', 'draft').order('created_at', { ascending: false }).limit(1).single(),
        supabase.from('orders').select('*, order_lines(count)').eq('user_id', session.user.id).eq('area', 'boh').eq('status', 'ready').order('created_at', { ascending: false }).limit(1).single(),
      ])
  ...
  const resumeOrder = (order) => {
    router.push(`/boh/ordering/order?resume=${order.id}`)
  }
          ...
          <div style={{ background: '#EAF3DE', border: '1px solid #97C459', borderRadius: '12px', padding: isMobile ? '14px' : '16px 20px', marginBottom: '12px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '12px' }}>
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: '12px', color: '#3B6D11' }}>
                ...
                {!isMobile && ' · Ready to submit to your vendors'}
              </div>
            </div>
            <div style={{ display: 'flex', gap: '8px', flexShrink: 0 }}>
              ...
              {pendingOrders.map(order => {
                ...
                return (
                  <div key={order.id}
                    ...
                    <button onClick={() => router.push(`/boh/ordering/receive/${order.id}`)}
