...
export default function BOHReceiveOrder() {
  ...
  const loadOrder = async (userId) => {
    ...
    setOrder(orderData)
    setLines((lineData || []).map(l => ({ ...l, received: true, received_qty: l.final_qty })))

    const recMap = {}
    ;(receivingData || []).forEach(r => { recMap[r.distributor_name] = r })
    ...
    if (!receivingData || receivingData.length === 0) {
      ...
      const toInsert = distNames.map(name => {
        ...
        return { order_id: params.orderId, user_id: userId, distributor_id: distLine?.distributor_id || null, distributor_name: name, status: 'pending' }
      })
  ...
  const confirmDistributor = async (distName, status) => {
    ...
    // Update order_lines status only — no on_hand writes
    ...
    if (allDone) {
      const overallStatus = Object.values(newReceiving).every(r => r.status === 'received') ? 'received' : 'partial'
  ...
  const reopenDistributor = async (distName) => {
    const recRow = receiving[distName]
    const { data: updatedRec } = await supabase.from('order_receiving').update({
      status: 'pending', confirmed_at: null, is_reopened: true
    }).eq('id', recRow.id).select().single()
    ...
    <div style={{ minHeight: '100vh', background: '#f5f5f3', fontFamily: '-apple-system, BlinkMacSystemFont, Segoe UI, sans-serif' }}>
      ...
      <div style={{ padding: isMobile ? '16px' : '28px 24px', maxWidth: '900px', margin: '0 auto' }}>
        ...
        <div style={{ background: '#f0f8ff', border: '1px solid #b5d4f4', borderRadius: '10px', padding: '10px 14px', marginBottom: '16px', fontSize: '12px', color: '#185FA5' }}>
          💡 Confirm each vendor's delivery separately. Inventory updates when you scan the invoice — this just tracks what arrived.
        </div>

        {Object.entries(grouped).map(([vendorName, vendorLines]) => {
          const rec = receiving[vendorName]
          const isConfirmed = rec && rec.status !== 'pending'
          const isSubmitting = submitting === vendorName
          const badge = rec ? statusBadge(rec.status) : null

          return (
            <div key={vendorName} style={{ marginBottom: '20px', opacity: isConfirmed ? 0.85 : 1 }}>
              ...
              <div style={{ background: isConfirmed ? '#333' : '#111', borderRadius: '10px 10px 0 0', padding: '12px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                  ...
                  {isConfirmed && (
                    <button onClick={() => reopenDistributor(vendorName)}
                ...
                {isMobile ? (
                  vendorLines.map(line => (
                    <div key={line.id} style={{ padding: '14px 16px', borderBottom: '1px solid #f5f5f5', background: !line.received ? '#fff8f8' : 'transparent', opacity: isConfirmed ? 0.7 : 1 }}>
                      <div style={{ display: 'flex', alignItems: 'flex-start', gap: '10px', marginBottom: line.received ? '10px' : '0' }}>
                        <input type="checkbox" checked={line.received} onChange={() => !isConfirmed && toggleLine(line.id)} disabled={isConfirmed}
                          style={{ width: '20px', height: '20px', cursor: isConfirmed ? 'default' : 'pointer', accentColor: '#F5B800', flexShrink: 0, marginTop: '2px' }} />
                        <div style={{ flex: 1 }}>
                      ...
                      {line.received && (
                        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', paddingLeft: '30px' }}>
                          ...
                          <input type="number" min="0" step="0.1" value={line.received_qty === 0 ? '' : line.received_qty}
                            onChange={e => !isConfirmed && updateQty(line.id, parseFloat(e.target.value) || 0)} disabled={isConfirmed}
              ...
              {!isConfirmed && (
                <div style={{ background: '#fff', border: '1px solid #e8e8e8', borderTop: 'none', borderRadius: '0 0 10px 10px', padding: isMobile ? '14px' : '16px 20px' }}>
                  <div style={{ fontSize: '12px', color: '#aaa', marginBottom: '10px', textAlign: 'center' }}>
                    How did {vendorName}'s delivery go?
                  </div>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: isMobile ? '8px' : '12px' }}>
                    <button onClick={() => confirmDistributor(vendorName, 'rejected')} disabled={!!isSubmitting}
                    ...
                    <button onClick={() => confirmDistributor(vendorName, 'partial')} disabled={!!isSubmitting}
                    ...
                    <button onClick={() => confirmDistributor(vendorName, 'received')} disabled={!!isSubmitting}
        ...
        <div style={{ background: '#fff', border: '1px solid #e8e8e8', borderRadius: '12px', padding: '16px 20px', textAlign: 'center' }}>
          {confirmedDistributors < totalDistributors ? (
            <div style={{ fontSize: '13px', color: '#aaa' }}>
              {totalDistributors - confirmedDistributors} vendor{totalDistributors - confirmedDistributors !== 1 ? 's' : ''} still pending confirmation
            </div>
          ) : (
            <div style={{ fontSize: '13px', color: '#3B6D11', fontWeight: '500' }}>
              ✓ All vendors confirmed — order will close automatically
            </div>
          )}
        </div>
