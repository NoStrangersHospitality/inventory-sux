...
const DRY_UNITS = [{ value: 'g', label: 'Gram (g)' }, { value: 'oz_w', label: 'Ounce — weight (oz)' }, { value: 'lb', label: 'Pound (lb)' }]
const LIQUID_UNITS = [{ value: 'ml', label: 'Milliliter (ml)' }, { value: 'fl_oz', label: 'Fluid Ounce (fl oz)' }, { value: 'tsp', label: 'Teaspoon (tsp)' }, { value: 'tbsp', label: 'Tablespoon (tbsp)' }, { value: 'cup', label: 'Cup' }, { value: 'pt', label: 'Pint (pt)' }, { value: 'qt', label: 'Quart (qt)' }, { value: 'L', label: 'Liter (L)' }]
...
const IngForm = ({ form, setForm, saving, isMobile, editingId, onSave, onCancel }) => {
  ...
  const mock = { purchase_qty: parseFloat(form.purchase_qty), purchase_unit: form.purchase_unit, purchase_cost: parseFloat(form.purchase_cost), density: parseFloat(form.density) || null }
  const cpg = form.purchase_qty && form.purchase_cost ? ingCostPerG(mock) : null
  const cpml = form.purchase_qty && form.purchase_cost ? ingCostPerML(mock) : null
  return (
    <div style={{ background: '#fff', border: '1px solid #e8e8e8', borderRadius: '12px', padding: isMobile ? '16px' : '24px', marginBottom: '16px' }}>
      <h3 style={{ fontSize: '15px', fontWeight: '500', color: '#000', marginBottom: '14px' }}>{editingId ? 'Edit Ingredient' : 'Add Ingredient'}</h3>
      <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: '12px', marginBottom: '12px' }}>
        <div style={{ gridColumn: '1/-1' }}><label style={labelStyle}>Ingredient Name</label><input style={inputStyle} placeholder="Smoked Paprika" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} /></div>
        ...
        <div><label style={labelStyle}>Type</label><select style={inputStyle} value={form.item_type} onChange={e => setForm(f => ({ ...f, item_type: e.target.value, purchase_unit: e.target.value === 'dry' ? 'lb' : 'L' }))}><option value="dry">Dry / Weight</option><option value="liquid">Liquid / Volume</option></select></div>
        <div><label style={labelStyle}>Purchase Quantity</label><input style={inputStyle} type="number" step="0.01" placeholder="16" value={form.purchase_qty} onChange={e => setForm(f => ({ ...f, purchase_qty: e.target.value }))} /></div>
        <div><label style={labelStyle}>Purchase Unit</label><select style={inputStyle} value={form.purchase_unit} onChange={e => setForm(f => ({ ...f, purchase_unit: e.target.value }))}>{(form.item_type === 'dry' ? DRY_UNITS : LIQUID_UNITS).map(u => <option key={u.value} value={u.value}>{u.label}</option>)}</select></div>
        <div><label style={labelStyle}>Purchase Cost ($)</label><input style={inputStyle} type="number" step="0.01" placeholder="8.99" value={form.purchase_cost} onChange={e => setForm(f => ({ ...f, purchase_cost: e.target.value }))} /></div>
        {form.item_type === 'dry' && <div><label style={labelStyle}>Density (g/ml) <span style={{ color: '#aaa', fontSize: '10px', fontWeight: 400 }}>optional</span></label><input style={inputStyle} type="number" step="0.001" placeholder="water=1.0, flour≈0.53" value={form.density} onChange={e => setForm(f => ({ ...f, density: e.target.value }))} /></div>}
        <div><label style={labelStyle}>Vendor</label><input style={inputStyle} placeholder="Sysco, US Foods..." value={form.vendor} onChange={e => setForm(f => ({ ...f, vendor: e.target.value }))} /></div>
        <div style={{ gridColumn: '1/-1' }}><label style={labelStyle}>Notes</label><input style={inputStyle} placeholder="Brand, spec, storage..." value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))} /></div>
      </div>
      {cpg !== null || cpml !== null ? (
        <div style={{ background: '#fafafa', border: '1px solid #e8e8e8', borderRadius: '10px', padding: '14px', marginBottom: '12px', display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '12px', textAlign: 'center' }}>
          {[{ label: 'Cost per gram', val: cpg ? ('$' + cpg.toFixed(5)) : '--' }, { label: 'Cost per ml', val: cpml ? ('$' + cpml.toFixed(5)) : '--' }, { label: 'Cost per oz (wt)', val: cpg ? ('$' + (cpg * 28.3495).toFixed(4)) : '--', gold: true }].map(p => <div key={p.label}><div style={{ fontSize: '11px', color: p.gold ? '#F5B800' : '#aaa', marginBottom: '3px' }}>{p.label}</div><div style={{ fontSize: isMobile ? '13px' : '16px', fontWeight: '700', color: p.gold ? '#F5B800' : '#000' }}>{p.val}</div></div>)}
        </div>
      ) : null}
...
const PrepIngForm = ({ form, setForm, saving, isMobile, editingId, onSave, onCancel }) => {
  ...
  return (
    <div style={{ background: '#fff', border: '1px solid #e8e8e8', borderRadius: '12px', padding: isMobile ? '16px' : '24px', marginBottom: '16px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: '12px', marginBottom: '12px' }}>
        <div style={{ gridColumn: '1/-1' }}><label style={labelStyle}>Ingredient Name</label><input style={inputStyle} placeholder="White Sugar" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} /></div>
...
export default function BOHCOGS() {
  ...
  const saveIngForm = async () => {
    ...
    if (editingIngId) { await supabase.from('ingredients').update(payload).eq('id', editingIngId) }
    else { await supabase.from('ingredients').insert(payload) }
    await loadData(session.user.id); setShowIngForm(false); setSaving(false)
  }
  ...
  const exportIngredients = () => {
    ...
    const a = document.createElement('a'); a.href = URL.createObjectURL(new Blob([csv], { type: 'text/csv' })); a.download = 'boh_ingredients.csv'; a.click()
  }

  const downloadIngsTemplate = () => {
    const rows = [['Name', 'Category', 'Type', 'Purchase Qty', 'Purchase Unit', 'Purchase Cost ($)', 'Density (g/ml)', 'Vendor', 'Notes'], ['Smoked Paprika', 'Dry Spices', 'dry', '1', 'lb', '8.99', '', 'Sysco', ''], ['Heavy Cream', 'Dairy', 'liquid', '1', 'L', '4.50', '1.01', 'US Foods', ''], ['Chicken Breast', 'Proteins', 'dry', '1', 'lb', '3.99', '', 'Sysco', 'boneless skinless']]
    const csv = rows.map(r => r.map(c => `"${c}"`).join(',')).join('\n')
    const a = document.createElement('a'); a.href = URL.createObjectURL(new Blob([csv], { type: 'text/csv' })); a.download = 'boh_ingredients_template.csv'; a.click()
  }
  ...
  const handleIngsImport = (e) => {
    ...
    reader.onload = (ev) => {
      ...
      const ni = hdr.findIndex(h => h.includes('name')), cati = hdr.findIndex(h => h.includes('cat')), ti = hdr.findIndex(h => h.includes('type')), pqi = hdr.findIndex(h => h.includes('qty') || h.includes('quantity')), pui = hdr.findIndex(h => h.includes('unit')), pci = hdr.findIndex(h => h.includes('cost')), di = hdr.findIndex(h => h.includes('density')), vi = hdr.findIndex(h => h.includes('vendor')), noi = hdr.findIndex(h => h.includes('note'))
      const parsed = []
      for (let i = 1; i < lines.length; i++) {
        ...
        parsed.push({ name, category: cols[cati >= 0 ? cati : 1] || '', item_type, purchase_qty, purchase_unit: cols[pui >= 0 ? pui : 4] || (item_type === 'dry' ? 'lb' : 'L'), purchase_cost, density: parseFloat(cols[di >= 0 ? di : 6]) || null, vendor: cols[vi >= 0 ? vi : 7] || '', notes: cols[noi >= 0 ? noi : 8] || '', valid: purchase_qty > 0 && purchase_cost > 0 })
      }
  ...
  const confirmIngsImport = async () => {
    ...
    if (toInsert.length > 0) {
      await supabase.from('ingredients').insert(toInsert.map(r => ({ name: r.name, category: r.category, item_type: r.item_type, purchase_qty: r.purchase_qty, purchase_unit: r.purchase_unit, purchase_cost: r.purchase_cost, density: r.density, vendor: r.vendor, notes: r.notes, user_id: session.user.id })))
      await loadData(session.user.id)
    }
    setIngImportPreview(null); setImportingIngs(false)
  }
  ...
  const openRecipeForm = (recipe = null) => {
    if (recipe) {
      ...
      setRecipeForm({ name: '', menu_price: '', target_cost_pct: '0.30', yield_portions: '1' }); setEditingRecipeId(null)
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f5f5f3', fontFamily: '-apple-system, BlinkMacSystemFont, Segoe UI, sans-serif' }}>
      <div style={{ background: '#fff', borderBottom: '2px solid #F5B800', padding: isMobile ? '10px 16px' : '10px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <button onClick={() => view === 'hub' ? router.push('/boh') : setView('hub')} style={{ background: '#333', border: 'none', color: '#fff', padding: '6px 12px', borderRadius: '6px', fontSize: '12px', cursor: 'pointer' }}>
      ...
      <div style={{ padding: isMobile ? '16px' : '28px 24px', maxWidth: '1100px', margin: '0 auto' }}>
        ...
        {view === 'hub' && (
          <>
            <h1 style={{ fontSize: isMobile ? '18px' : '20px', fontWeight: '500', color: '#000', marginBottom: '6px' }}>BOH COGS</h1>
            <p style={{ color: '#999', fontSize: '13px', marginBottom: '20px' }}>Ingredient costing and kitchen recipe margins.</p>
            <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(3,1fr)', gap: isMobile ? '12px' : '20px' }}>
              {[
                ...
                { title: 'Prep Costing', desc: 'Stocks, sauces, batches, and more', icon: '🧪', view: 'prep' },
        ...
        {view === 'ingredients' && (
          <>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '14px', gap: '12px' }}>
              <div>
                <h1 style={{ fontSize: isMobile ? '17px' : '20px', fontWeight: '500', color: '#000' }}>Ingredients Database</h1>
                {!isMobile && <p style={{ color: '#999', fontSize: '13px', marginTop: '4px' }}>Purchase costs and unit conversions</p>}
              </div>
              <div style={{ display: 'flex', gap: '6px', flexShrink: 0 }}>
                {!isMobile && <>
                  <button onClick={downloadIngsTemplate} style={{ background: '#fff', color: '#555', border: '1px solid #e8e8e8', padding: '8px 12px', borderRadius: '8px', fontSize: '12px', cursor: 'pointer' }}>↓ Template</button>
                  {ingredients.length > 0 && <button onClick={exportIngredients} style={{ background: '#fff', color: '#555', border: '1px solid #e8e8e8', padding: '8px 12px', borderRadius: '8px', fontSize: '12px', cursor: 'pointer' }}>↓ Export</button>}
                  <label style={{ background: '#fff', color: '#555', border: '1px solid #e8e8e8', padding: '8px 12px', borderRadius: '8px', fontSize: '12px', cursor: 'pointer' }}>↑ Import<input type="file" accept=".csv" onChange={handleIngsImport} style={{ display: 'none' }} /></label>
                </>}
                <button onClick={() => { setIngForm({ name: '', category: '', item_type: 'dry', purchase_qty: '', purchase_unit: 'lb', purchase_cost: '', density: '', vendor: '', notes: '' }); setEditingIngId(null); setShowIngForm(true) }} style={{ background: '#333', color: '#fff', border: 'none', padding: '8px 12px', borderRadius: '8px', fontSize: '12px', fontWeight: '600', cursor: 'pointer' }}>+ Add</button>
              </div>
            </div>

            {isMobile && (
              <div style={{ display: 'flex', gap: '8px', marginBottom: '14px' }}>
                <button onClick={downloadIngsTemplate} style={{ flex: 1, background: '#fff', color: '#555', border: '1px solid #e8e8e8', padding: '8px', borderRadius: '8px', fontSize: '12px', cursor: 'pointer' }}>↓ Template</button>
                {ingredients.length > 0 && <button onClick={exportIngredients} style={{ flex: 1, background: '#fff', color: '#555', border: '1px solid #e8e8e8', padding: '8px', borderRadius: '8px', fontSize: '12px', cursor: 'pointer' }}>↓ Export</button>}
                <label style={{ flex: 1, background: '#fff', color: '#555', border: '1px solid #e8e8e8', padding: '8px', borderRadius: '8px', fontSize: '12px', cursor: 'pointer', textAlign: 'center' }}>↑ Import<input type="file" accept=".csv" onChange={handleIngsImport} style={{ display: 'none' }} /></label>
              </div>
            )}
              ...
              <div style={{ background: '#fff', border: '1px solid #e8e8e8', borderRadius: '12px', padding: '16px', marginBottom: '16px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px', gap: '12px' }}>
                  <div>
                    ...
                    <div style={{ fontSize: '12px', color: '#aaa', marginTop: '2px' }}>{ingImportPreview.filter(r => r.valid).length} valid · {ingImportPreview.filter(r => r.valid && !ingredients.map(i => i.name.toLowerCase()).includes(r.name.toLowerCase())).length} new</div>
                  </div>
                  <div style={{ display: 'flex', gap: '8px' }}>
                    <button onClick={() => setIngImportPreview(null)} style={{ background: '#fff', color: '#555', border: '1px solid #e8e8e8', padding: '7px 12px', borderRadius: '8px', fontSize: '12px', cursor: 'pointer' }}>Cancel</button>
                    <button onClick={confirmIngsImport} disabled={importingIngs} style={{ background: importingIngs ? '#ccc' : '#F5B800', color: '#000', border: 'none', padding: '7px 16px', borderRadius: '8px', fontSize: '13px', fontWeight: '700', cursor: importingIngs ? 'not-allowed' : 'pointer' }}>{importingIngs ? 'Importing...' : 'Confirm'}</button>
                  </div>
                </div>
                {isMobile ? (
                    ...
                    {ingImportPreview.map((r, i) => {
                      ...
                      return (
                        <div key={i} style={{ background: '#fafafa', borderRadius: '8px', padding: '10px 12px', border: '1px solid #f0f0f0' }}>
                          ...
                          <div style={{ fontSize: '11px', color: '#aaa' }}>{r.item_type} · {r.purchase_qty} {r.purchase_unit} · {r.purchase_cost ? fmt(r.purchase_cost) : '--'}</div>
                        </div>
                      )
                    })}
                  </div>
                  ...
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px' }}>
                    <thead><tr>{['Name', 'Category', 'Type', 'Purchase', 'Cost', 'Vendor', 'Status'].map((h, i) => <th key={i} style={{ textAlign: 'left', fontSize: '10px', color: '#aaa', textTransform: 'uppercase', padding: '6px 10px', borderBottom: '1px solid #f0f0f0', background: '#fafafa' }}>{h}</th>)}</tr></thead>
                    <tbody>
                      {ingImportPreview.map((r, i) => {
                        const exists = ingredients.map(ing => ing.name.toLowerCase()).includes(r.name.toLowerCase())
                        return (
                          <tr key={i} style={{ borderBottom: '1px solid #f8f8f8' }}>
                            ...
                            <td style={{ padding: '7px 10px' }}>{!r.valid ? <span style={{ color: '#E24B4A', fontSize: '11px' }}>Missing qty/cost</span> : exists ? <span style={{ color: '#aaa', fontSize: '11px' }}>Exists</span> : <span style={{ color: '#3B6D11', fontSize: '11px' }}>✓ New</span>}</td>
                          </tr>
                        )
                      })}
                    </tbody>
            ...
            {showIngForm && <IngForm form={ingForm} setForm={setIngForm} saving={saving} isMobile={isMobile} editingId={editingIngId} onSave={saveIngForm} onCancel={() => setShowIngForm(false)} />}
            ...
            {ingredients.length === 0 ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                {ingredients.map(ing => {
                  ...
                  const cpml = ingCostPerML(ing)
                  return (
                    <div key={ing.id} style={{ background: '#fff', border: '1px solid #e8e8e8', borderRadius: '12px', padding: '14px 16px' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '8px' }}>
                        <div style={{ flex: 1, marginRight: '12px' }}>
                          <div style={{ fontSize: '14px', fontWeight: '500', color: '#000', marginBottom: '2px' }}>{ing.name}</div>
                          <div style={{ fontSize: '11px', color: '#aaa' }}>{ing.category || '--'} · {ing.item_type === 'dry' ? 'Dry' : 'Liquid'}{ing.vendor ? ` · ${ing.vendor}` : ''}</div>
                        </div>
                        <button onClick={() => { setIngForm({ name: ing.name, category: ing.category || '', item_type: ing.item_type || 'dry', purchase_qty: ing.purchase_qty, purchase_unit: ing.purchase_unit, purchase_cost: ing.purchase_cost, density: ing.density || '', vendor: ing.vendor || '', notes: ing.notes || '' }); setEditingIngId(ing.id); setShowIngForm(true) }} style={{ background: '#333', border: 'none', color: '#fff', padding: '6px 12px', borderRadius: '6px', fontSize: '11px', cursor: 'pointer', flexShrink: 0 }}>Edit</button>
                      </div>
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '8px' }}>
                        <div><div style={{ fontSize: '10px', color: '#aaa', marginBottom: '2px', textTransform: 'uppercase' }}>Purchase</div><div style={{ fontSize: '12px', fontWeight: '500', color: '#555' }}>{ing.purchase_qty} {ing.purchase_unit}</div></div>
                        <div><div style={{ fontSize: '10px', color: '#aaa', marginBottom: '2px', textTransform: 'uppercase' }}>Cost</div><div style={{ fontSize: '13px', fontWeight: '600', color: '#000' }}>{fmt(ing.purchase_cost)}</div></div>
                        <div><div style={{ fontSize: '10px', color: '#aaa', marginBottom: '2px', textTransform: 'uppercase' }}>Per unit</div><div style={{ fontSize: '11px', fontWeight: '600', color: '#F5B800' }}>{cpg ? ('$' + cpg.toFixed(4) + '/g') : (cpml ? ('$' + cpml.toFixed(4) + '/ml') : '--')}</div></div>
                      </div>
                    </div>
                  )
                })}
          ...
          <>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '14px', gap: '12px' }}>
              <div>
                ...
                {!isMobile && <p style={{ color: '#999', fontSize: '13px', marginTop: '4px' }}>Stocks, sauces, batches, and more — shared with FOH</p>}
              </div>
        ...
        {view === 'recipes' && (
          <>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '16px' }}>
              <div>
                <h1 style={{ fontSize: isMobile ? '17px' : '20px', fontWeight: '500', color: '#000' }}>Kitchen Recipes</h1>
                {!isMobile && <p style={{ color: '#999', fontSize: '13px', marginTop: '4px' }}>Food recipe costing and margin analysis</p>}
              </div>
            ...
            {showRecipeForm && (
              <div style={{ background: '#fff', border: '1px solid #e8e8e8', borderRadius: '12px', padding: isMobile ? '16px' : '24px', marginBottom: '16px' }}>
                <h3 style={{ fontSize: '15px', fontWeight: '500', color: '#000', marginBottom: '14px' }}>{editingRecipeId ? 'Edit Recipe' : 'Add Kitchen Recipe'}</h3>
                ...
                {recipeIngs.map((ri, i) => (
                  <div key={i} style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr 60px 60px 28px' : '2fr 1fr 1fr auto', gap: '8px', marginBottom: '8px', alignItems: 'center' }}>
                    ...
                    <button onClick={() => setRecipeIngs(prev => prev.length > 1 ? prev.filter((_, j) => j !== i) : prev)} style={{ background: '#333', border: 'none', color: '#fff', width: '28px', height: '36px', borderRadius: '6px', cursor: 'pointer', fontSize: '14px' }}>×</button>
                  </div>
                ))}
