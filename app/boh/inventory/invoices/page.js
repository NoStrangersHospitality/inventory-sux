    ...
    for (const line of actionableLines) {
      ...
      if (isCreate || !invItem) {
        ...
        await supabase.from('invoice_lines').insert({
          ...
          matched_item_id: newItem.id, qty: (parseFloat(line.qty) || 0) * (line.case_size || 1),
          unit_cost: line.case_size > 1 ? (parseFloat(line.unit_cost) || 0) / (line.case_size || 1) : (parseFloat(line.unit_cost) || 0),
          total_cost: parseFloat(line.total_cost) || 0, match_confidence: 1, match_status: 'matched'
        })
        await supabase.from('inventory_history').insert({
          user_id: session.user.id, inventory_item_id: newItem.id, item_name: newItem.name,
          category: newItem.category, area: 'boh', event_type: 'invoice', event_id: scanResult.invoice_id,
        ...
        const newUnitCost = line.unit_cost ? (line.case_size > 1 ? (parseFloat(line.unit_cost) || 0) / (line.case_size || 1) : parseFloat(line.unit_cost)) : invItem.unit_cost
        ...
        await supabase.from('invoice_lines').insert({
          invoice_id: scanResult.invoice_id, user_id: session.user.id, raw_name: line.raw_name,
          matched_item_id: invItem.id, qty: (parseFloat(line.qty) || 0) * (line.case_size || 1),
          unit_cost: newUnitCost, total_cost: parseFloat(line.total_cost) || 0,
        ...
        await supabase.from('inventory_history').insert({
          ...
          category: invItem.category, area: 'boh', event_type: 'invoice', event_id: scanResult.invoice_id,
          quantity_before: parseFloat(invItem.on_hand) || 0, quantity_change: (parseFloat(line.qty) || 0) * (line.case_size || 1),
          ...
          <>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px', gap: '12px' }}>
              <div>
                ...
                {!isMobile && <p style={{ color: '#999', fontSize: '13px', marginTop: '4px' }}>Scan a vendor invoice to automatically update kitchen inventory.</p>}
              </div>
