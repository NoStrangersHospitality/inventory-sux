  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f5f5f3', fontFamily: '-apple-system, BlinkMacSystemFont, Segoe UI, sans-serif' }}>
      ...
      <div style={{ background: '#fff', borderBottom: '2px solid #F5B800', padding: '10px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <button onClick={() => router.push('/boh/inventory')} style={{ background: '#333', border: 'none', color: '#fff', padding: '6px 14px', borderRadius: '6px', fontSize: '12px', cursor: 'pointer' }}>← Inventory</button>
      </div>
      ...
      <div style={{ padding: '28px 24px', maxWidth: '1100px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px' }}>
          <div>
            ...
            <p style={{ color: '#999', fontSize: '13px', marginTop: '4px' }}>Every ingredient and supply — on hand, value, and par.</p>
          </div>
          ...
          {CATEGORIES.map(c => {
            ...
            return (
              <div key={c.key} onClick={() => { setActiveCategory(c.key); setImportPreview(null); cancelEdit() }}
                ...
                <div style={{ fontSize: '10px', color: '#aaa' }}>{count} items · {fmt(val)}</div>
              </div>
            )
          })}
        </div>
          ...
          <div style={{ background: '#fff', border: '1px solid #e8e8e8', borderRadius: '12px', padding: '24px', marginBottom: '20px' }}>
            ...
            {form.on_hand > 0 && form.unit_cost > 0 && (
              <div style={{ background: '#fafafa', border: '1px solid #e8e8e8', borderRadius: '10px', padding: '12px 16px', marginBottom: '14px', display: 'flex', gap: '24px' }}>
                ...
                {form.par > 0 && <div><div style={{ fontSize: '11px', color: '#aaa', marginBottom: '2px' }}>Variance</div><div style={{ fontSize: '16px', fontWeight: '700', color: parseFloat(form.on_hand) >= parseFloat(form.par) ? '#3B6D11' : '#E24B4A' }}>{(parseFloat(form.on_hand) - parseFloat(form.par)).toFixed(2)}</div></div>}
              </div>
            )}
