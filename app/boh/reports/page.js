...
export default function BOHReports() {
  ...
  const runReport = async () => {
    ...
    const { data: invoices } = await supabase.from('invoices').select('*, invoice_lines(*)')
      .eq('user_id', userId).eq('area', 'boh').eq('status', 'confirmed')
    ...
    <div style={{ minHeight: '100vh', background: '#f5f5f3', fontFamily: '-apple-system, BlinkMacSystemFont, Segoe UI, sans-serif' }}>
      ...
      <div style={{ padding: isMobile ? '16px' : '28px 24px', maxWidth: '1100px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px' }}>
          <div>
            <h1 style={{ fontSize: isMobile ? '18px' : '20px', fontWeight: '500', color: '#000' }}>BOH Reports</h1>
        ...
        {sessions.length < 2 ? (
          <div style={{ background: '#fff', border: '1px solid #e8e8e8', borderRadius: '12px', padding: '48px', textAlign: 'center' }}>
            ...
            <button onClick={() => router.push('/boh/inventory/count')} style={{ background: '#333', color: '#fff', border: 'none', padding: '10px 24px', borderRadius: '8px', fontSize: '13px', fontWeight: '600', cursor: 'pointer' }}>Start a Count →</button>
          </div>
        ) : (
          <>
            {/* Trend chart */}
                  ...
                  <div style={{ background: '#fff', border: '1px solid #e8e8e8', borderRadius: '12px', padding: isMobile ? '14px 16px' : '20px 24px', marginBottom: '16px' }}>
                    ...
                    <ResponsiveContainer width="100%" height={isMobile ? 150 : 220}>
                      <BarChart data={categoryTrend} margin={{ left: 0, right: 8, top: 0, bottom: 0 }}>
                        <XAxis dataKey="category" tick={{ fontSize: isMobile ? 9 : 11, fill: '#555' }} axisLine={false} tickLine={false} />
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px', gap: '8px', flexWrap: 'wrap' }}>
                  ...
                  <div style={{ display: 'flex', gap: '5px', flexWrap: 'wrap', justifyContent: 'flex-end' }}>
                    <button onClick={() => setFilterCategory('all')}
                      style={{ padding: '5px 8px', border: '1px solid', borderColor: filterCategory === 'all' ? '#333' : '#e8e8e8', borderRadius: '20px', fontSize: '10px', fontWeight: filterCategory === 'all' ? '600' : '400', cursor: 'pointer', background: filterCategory === 'all' ? '#333' : '#fff', color: filterCategory === 'all' ? '#fff' : '#666' }}>
                      All
                    </button>
                    {CATEGORIES.map(c => (
                      <button key={c.key} onClick={() => setFilterCategory(c.key)}
                        style={{ padding: '5px 8px', border: '1px solid', borderColor: filterCategory === c.key ? '#333' : '#e8e8e8', borderRadius: '20px', fontSize: '10px', fontWeight: filterCategory === c.key ? '600' : '400', cursor: 'pointer', background: filterCategory === c.key ? '#333' : '#fff', color: filterCategory === c.key ? '#fff' : '#666' }}>
                        {isMobile ? c.label.split(' ')[0].slice(0, 4) : c.label}
                      </button>
                    ))}
                  </div>
                </div>
                        ...
                        <div key={i} style={{ background: item.variance < -1 ? '#fff8f8' : '#fff', border: `1px solid ${item.variance < -1 ? '#fca5a5' : '#e8e8e8'}`, borderRadius: '12px', padding: '14px 16px' }}>
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '8px' }}>
                            ...
                            <div style={{ textAlign: 'right' }}>
                              <div style={{ fontSize: '14px', fontWeight: '700', color: item.varianceCost >= 0 ? '#3B6D11' : '#E24B4A' }}>{item.varianceCost >= 0 ? '+' : ''}{fmt(item.varianceCost)}</div>
                              <div style={{ fontSize: '11px', color: item.variance >= 0 ? '#3B6D11' : '#E24B4A', fontWeight: '500' }}>{item.variance >= 0 ? '+' : ''}{item.variance.toFixed(2)} units</div>
                            </div>
                          </div>
